  ...
  #  Capture data (latest download: 11.16.20)
  #  Note the "ï.."
  md_cap <- read.csv("./Data/Capture (MD)_11.16.20.csv", stringsAsFactors = FALSE) %>%
    mutate(IndividualIdentifier = ï..IndividualIdentifier) %>%
    select(-ï..IndividualIdentifier)
  elk_cap <- read.csv("./Data/Capture (Elk)_11.16.20.csv", stringsAsFactors = FALSE) %>%
    mutate(IndividualIdentifier = ï..IndividualIdentifier) %>%
    select(-ï..IndividualIdentifier)
  wtd_cap <- read.csv("./Data/Capture (WTD)_11.16.20.csv", stringsAsFactors = FALSE) %>%
    mutate(IndividualIdentifier = ï..IndividualIdentifier) %>%
    select(-ï..IndividualIdentifier) 
  
  #str(md_cap)#; head(md_cap)
  
  #  Mortality data (latest download: 11.16.20)
  md_mort <- read.csv("./Data/Mortality (MD)_11.16.20.csv", stringsAsFactors = FALSE) 
  elk_mort <- read.csv("./Data/Mortality (Elk)_11.16.20.csv", stringsAsFactors = FALSE) 
  wtd_mort <- read.csv("./Data/Mortality (WTD)_11.16.20.csv", stringsAsFactors = FALSE) 
  ...
  elk_tel <- read.csv("./Data/dev_telem_elk_11.16.20.csv") %>%  
    mutate(daytime = mdy_hms(ObservationDateTimePST, tz = "America/Los_Angeles"),
           UTCdt = with_tz(daytime, "UTC"),
           Finaldt = with_tz(UTCdt, tzone = "Etc/GMT+8"),
           Floordt = floor_date(Finaldt, unit = "hour")) %>%
