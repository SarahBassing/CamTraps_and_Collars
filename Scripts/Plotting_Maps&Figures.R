 ...
 #'    coord_sf(xlim = c(490000.0, 780000.0), ylim = c(102000.0, 250000.0))
  ...
  #' 
  #' #'  Scaling covariates differently for different models!
  #' #'  ALL covariate values, scaled separately by study area and then combined
  #' #'  For Ungulate RSFs since data from only 1 study area was included in models
  #' OK_covs_scale <- read.csv("./Outputs/Tables/StudyAreaWide_OK_Covariates_2021-08-10.csv") %>%
  #'   dplyr::select(-X) %>%
  #'   transmute(
  #'     obs = obs,
  #'     Elev = scale(Elev),
  ...
  #'     y = y
  #'   )
  #' NE_covs_scale <- read.csv("./Outputs/Tables/StudyAreaWide_NE_Covariates_2021-08-10.csv") %>%
  #'   dplyr::select(-X) %>%
  #'   transmute(
  #'     obs = obs,
  #'     Elev = scale(Elev),
  ...
  #'     y = y
  #'   )
  #' all_covs <- as.data.frame(rbind(NE_covs_scale, OK_covs_scale))
  #' 
  #' #'  All covariate values combined, then scaled across both study areas at once 
  #' #'  For predator RSFs since data from both study areas were modeled together
  #' #'  (Although available locations were specific to study area)
  ...
  #'   dplyr::select(-X) %>%
  #'   mutate(Area = 0)
  #' combo_covs <- as.data.frame(rbind(OK_covs, NE_covs)) %>%
  #'   transmute(
  #'     obs = obs,
  #'     Elev = scale(Elev),
  #'     Slope = scale(Slope),
  ...
  #'   )
  #'   
  #' #'  Exclude values over 2150 m elevation from data for predictions based on
  #' #'  camera data since we did not sample above this elevation (Year1 & Year2)
  #' #'  Scale covariates separately by study area and then combined for Ungulate OccMods
  #' OK_covs_adj_scale <- read.csv("./Outputs/Tables/StudyAreaWide_OK_Covariates_2021-08-10.csv") %>%
  #'   dplyr::select(-X) %>%
  #'   mutate(
  #'     Elev = ifelse(Elev > 2150, NA, Elev)
  #'   ) %>%
  #'   transmute(
  #'     obs = obs,
  #'     Elev = scale(Elev),
  ...
  #'     y = y
  #'   )
  #' NE_covs_adj_scale <- read.csv("./Outputs/Tables/StudyAreaWide_NE_Covariates_2021-08-10.csv") %>%
  #'   dplyr::select(-X) %>%
  #'   mutate(
  #'     Elev = ifelse(Elev > 2150, NA, Elev)
  #'   ) %>%
  ...
  #'     y = y
  #'   )
  #' adj_covs <- as.data.frame(rbind(NE_covs_adj_scale, OK_covs_adj_scale))
  #' 
  #' #'  Exclude elevations >2150 and combine adjusted covariate values, 
  #' #'  then scale across both study areas at once for predator OccMods
  #' OK_covs_adj <- read.csv("./Outputs/Tables/StudyAreaWide_OK_Covariates_2021-08-10.csv") %>%
  #'   dplyr::select(-X) %>%
  #'   mutate(
  #'     Elev = ifelse(Elev > 2150, NA, Elev),
  #'     Area = 1
  #'     )
  #' NE_covs_adj <- read.csv("./Outputs/Tables/StudyAreaWide_NE_Covariates_2021-08-10.csv") %>%
  #'   dplyr::select(-X) %>%
  #'   mutate(
  #'     Elev = ifelse(Elev > 2150, NA, Elev),
  #'     Area = 0
  #'     )
  #' combo_covs_adj <- as.data.frame(rbind(OK_covs_adj, NE_covs_adj)) %>%
  #'   transmute(
  #'     obs = obs,
  #'     Elev = scale(Elev),
  #'     Slope = scale(Slope),
  ...
  #'   # covs <- cbind(covs, xy_saproj)
  #' }
  #' all_covs <- sp_covs(all_covs)
  #' combo_covs <- sp_covs(combo_covs)
  #' adj_covs <- sp_covs(adj_covs)
  #' combo_covs_adj <- sp_covs(combo_covs_adj)
  #' 
  ...
  #' md_smr_predict_occ <- predict_occ(OK_covs_adj_scale, occ_coefs[occ_coefs$Species == "Mule Deer" & occ_coefs$Season == "Summer",])
  #' md_wtr_predict_occ <- predict_occ(OK_covs_adj_scale, occ_coefs[occ_coefs$Species == "Mule Deer" & occ_coefs$Season == "Winter",])
  #' elk_smr_predict_occ <- predict_occ(NE_covs_adj_scale, occ_coefs[occ_coefs$Species == "Elk" & occ_coefs$Season == "Summer",])
  #' elk_wtr_predict_occ <- predict_occ(NE_covs_adj_scale, occ_coefs[occ_coefs$Species == "Elk" & occ_coefs$Season == "Winter",])
  #' wtd_smr_predict_occ <- predict_occ(NE_covs_adj_scale, occ_coefs[occ_coefs$Species == "White-tailed Deer" & occ_coefs$Season == "Summer",])
  #' wtd_wtr_predict_occ <- predict_occ(NE_covs_adj_scale, occ_coefs[occ_coefs$Species == "White-tailed Deer" & occ_coefs$Season == "Winter",])
  #' coug_smr_predict_occ <- predict_occ(combo_covs_adj, occ_coefs[occ_coefs$Species == "Cougar" & occ_coefs$Season == "Summer",])
  #' coug_wtr_predict_occ <- predict_occ(combo_covs_adj, occ_coefs[occ_coefs$Species == "Cougar" & occ_coefs$Season == "Winter",])
  #' wolf_smr_predict_occ <- predict_occ(combo_covs_adj, occ_coefs[occ_coefs$Species == "Wolf" & occ_coefs$Season == "Summer",])
  #' wolf_wtr_predict_occ <- predict_occ(combo_covs_adj, occ_coefs[occ_coefs$Species == "Wolf" & occ_coefs$Season == "Winter",])
  #' bob_smr_predict_occ <- predict_occ(combo_covs_adj, occ_coefs[occ_coefs$Species == "Bobcat" & occ_coefs$Season == "Summer",])
  #' bob_wtr_predict_occ <- predict_occ(combo_covs_adj, occ_coefs[occ_coefs$Species == "Bobcat" & occ_coefs$Season == "Winter",])
  #' coy_smr_predict_occ <- predict_occ(combo_covs_adj, occ_coefs[occ_coefs$Species == "Coyote" & occ_coefs$Season == "Summer",])
  #' coy_wtr_predict_occ <- predict_occ(combo_covs_adj, occ_coefs[occ_coefs$Species == "Coyote" & occ_coefs$Season == "Winter",])
  #' #'  Includes ONLY significant coefficients per species and season
  #' md_smr_predict_occ_sgnf <- predict_occ(OK_covs_adj_scale, occ_coefs_signif[occ_coefs_signif$Species == "Mule Deer" & occ_coefs_signif$Season == "Summer",])
  #' md_wtr_predict_occ_sgnf <- predict_occ(OK_covs_adj_scale, occ_coefs_signif[occ_coefs_signif$Species == "Mule Deer" & occ_coefs_signif$Season == "Winter",])
  #' elk_smr_predict_occ_sgnf <- predict_occ(NE_covs_adj_scale, occ_coefs_signif[occ_coefs_signif$Species == "Elk" & occ_coefs_signif$Season == "Summer",])
  #' elk_wtr_predict_occ_sgnf <- predict_occ(NE_covs_adj_scale, occ_coefs_signif[occ_coefs_signif$Species == "Elk" & occ_coefs_signif$Season == "Winter",])
  #' wtd_smr_predict_occ_sgnf <- predict_occ(NE_covs_adj_scale, occ_coefs_signif[occ_coefs_signif$Species == "White-tailed Deer" & occ_coefs_signif$Season == "Summer",])
  #' wtd_wtr_predict_occ_sgnf <- predict_occ(NE_covs_adj_scale, occ_coefs_signif[occ_coefs_signif$Species == "White-tailed Deer" & occ_coefs_signif$Season == "Winter",])
  #' coug_smr_predict_occ_sgnf <- predict_occ(combo_covs_adj, occ_coefs_signif[occ_coefs_signif$Species == "Cougar" & occ_coefs_signif$Season == "Summer",])
  #' coug_wtr_predict_occ_sgnf <- predict_occ(combo_covs_adj, occ_coefs_signif[occ_coefs_signif$Species == "Cougar" & occ_coefs_signif$Season == "Winter",])
  #' wolf_smr_predict_occ_sgnf <- predict_occ(combo_covs_adj, occ_coefs_signif[occ_coefs_signif$Species == "Wolf" & occ_coefs_signif$Season == "Summer",])
  #' wolf_wtr_predict_occ_sgnf <- predict_occ(combo_covs_adj, occ_coefs_signif[occ_coefs_signif$Species == "Wolf" & occ_coefs_signif$Season == "Winter",])
  #' bob_smr_predict_occ_sgnf <- predict_occ(combo_covs_adj, occ_coefs_signif[occ_coefs_signif$Species == "Bobcat" & occ_coefs_signif$Season == "Summer",])
  #' bob_wtr_predict_occ_sgnf <- predict_occ(combo_covs_adj, occ_coefs_signif[occ_coefs_signif$Species == "Bobcat" & occ_coefs_signif$Season == "Winter",])
  #' coy_smr_predict_occ_sgnf <- predict_occ(combo_covs_adj, occ_coefs_signif[occ_coefs_signif$Species == "Coyote" & occ_coefs_signif$Season == "Summer",])
  #' coy_wtr_predict_occ_sgnf <- predict_occ(combo_covs_adj, occ_coefs_signif[occ_coefs_signif$Species == "Coyote" & occ_coefs_signif$Season == "Winter",])
  #' 
  #' #'  Combine into a monster data frame
  #' #'  Start with predator data that spans both study areas
  #' Predicted_occ <- as.data.frame(combo_covs_adj) %>%
  ...
  #' md_smr_predict_rsf <- predict_rsf(OK_covs_scale, rsf_coefs[rsf_coefs$Species == "Mule Deer" & rsf_coefs$Season == "Summer",])
  #' md_wtr_predict_rsf <- predict_rsf(OK_covs_scale, rsf_coefs[rsf_coefs$Species == "Mule Deer" & rsf_coefs$Season == "Winter",])
  #' elk_smr_predict_rsf <- predict_rsf(NE_covs_scale, rsf_coefs[rsf_coefs$Species == "Elk" & rsf_coefs$Season == "Summer",])
  #' elk_wtr_predict_rsf <- predict_rsf(NE_covs_scale, rsf_coefs[rsf_coefs$Species == "Elk" & rsf_coefs$Season == "Winter",])
  #' wtd_smr_predict_rsf <- predict_rsf(NE_covs_scale, rsf_coefs[rsf_coefs$Species == "White-tailed Deer" & rsf_coefs$Season == "Summer",])
  #' wtd_wtr_predict_rsf <- predict_rsf(NE_covs_scale, rsf_coefs[rsf_coefs$Species == "White-tailed Deer" & rsf_coefs$Season == "Winter",])
  #' coug_smr_predict_rsf <- predict_rsf(combo_covs, rsf_coefs[rsf_coefs$Species == "Cougar" & rsf_coefs$Season == "Summer",])
  #' coug_wtr_predict_rsf <- predict_rsf(combo_covs, rsf_coefs[rsf_coefs$Species == "Cougar" & rsf_coefs$Season == "Winter",])
  #' wolf_smr_predict_rsf <- predict_rsf(combo_covs, rsf_coefs[rsf_coefs$Species == "Wolf" & rsf_coefs$Season == "Summer",])
  #' wolf_wtr_predict_rsf <- predict_rsf(combo_covs, rsf_coefs[rsf_coefs$Species == "Wolf" & rsf_coefs$Season == "Winter",])
  #' bob_smr_predict_rsf <- predict_rsf(combo_covs, rsf_coefs[rsf_coefs$Species == "Bobcat" & rsf_coefs$Season == "Summer",])
  #' bob_wtr_predict_rsf <- predict_rsf(combo_covs, rsf_coefs[rsf_coefs$Species == "Bobcat" & rsf_coefs$Season == "Winter",])
  #' coy_smr_predict_rsf <- predict_rsf(combo_covs, rsf_coefs[rsf_coefs$Species == "Coyote" & rsf_coefs$Season == "Summer",])
  #' coy_wtr_predict_rsf <- predict_rsf(combo_covs, rsf_coefs[rsf_coefs$Species == "Coyote" & rsf_coefs$Season == "Winter",])
  #' #'  Includes ONLY significant coefficients
  #' md_smr_predict_rsf_sgnf <- predict_rsf(OK_covs_scale, rsf_coefs_signif[rsf_coefs_signif$Species == "Mule Deer" & rsf_coefs_signif$Season == "Summer",])
  #' md_wtr_predict_rsf_sgnf <- predict_rsf(OK_covs_scale, rsf_coefs_signif[rsf_coefs_signif$Species == "Mule Deer" & rsf_coefs_signif$Season == "Winter",])
  #' elk_smr_predict_rsf_sgnf <- predict_rsf(NE_covs_scale, rsf_coefs_signif[rsf_coefs_signif$Species == "Elk" & rsf_coefs_signif$Season == "Summer",])
  #' elk_wtr_predict_rsf_sgnf <- predict_rsf(NE_covs_scale, rsf_coefs_signif[rsf_coefs_signif$Species == "Elk" & rsf_coefs_signif$Season == "Winter",])
  #' wtd_smr_predict_rsf_sgnf <- predict_rsf(NE_covs_scale, rsf_coefs_signif[rsf_coefs_signif$Species == "White-tailed Deer" & rsf_coefs_signif$Season == "Summer",])
  #' wtd_wtr_predict_rsf_sgnf <- predict_rsf(NE_covs_scale, rsf_coefs_signif[rsf_coefs_signif$Species == "White-tailed Deer" & rsf_coefs_signif$Season == "Winter",])
  #' coug_smr_predict_rsf_sgnf <- predict_rsf(combo_covs, rsf_coefs_signif[rsf_coefs_signif$Species == "Cougar" & rsf_coefs_signif$Season == "Summer",])
  #' coug_wtr_predict_rsf_sgnf <- predict_rsf(combo_covs, rsf_coefs_signif[rsf_coefs_signif$Species == "Cougar" & rsf_coefs_signif$Season == "Winter",])
  #' wolf_smr_predict_rsf_sgnf <- predict_rsf(combo_covs, rsf_coefs_signif[rsf_coefs_signif$Species == "Wolf" & rsf_coefs_signif$Season == "Summer",])
  #' wolf_wtr_predict_rsf_sgnf <- predict_rsf(combo_covs, rsf_coefs_signif[rsf_coefs_signif$Species == "Wolf" & rsf_coefs_signif$Season == "Winter",])
  #' bob_smr_predict_rsf_sgnf <- predict_rsf(combo_covs, rsf_coefs_signif[rsf_coefs_signif$Species == "Bobcat" & rsf_coefs_signif$Season == "Summer",])
  #' bob_wtr_predict_rsf_sgnf <- predict_rsf(combo_covs, rsf_coefs_signif[rsf_coefs_signif$Species == "Bobcat" & rsf_coefs_signif$Season == "Winter",])
  #' coy_smr_predict_rsf_sgnf <- predict_rsf(combo_covs, rsf_coefs_signif[rsf_coefs_signif$Species == "Coyote" & rsf_coefs_signif$Season == "Summer",])
  #' coy_wtr_predict_rsf_sgnf <- predict_rsf(combo_covs, rsf_coefs_signif[rsf_coefs_signif$Species == "Coyote" & rsf_coefs_signif$Season == "Winter",])
  #' 
  #' #'  Combine into a monster data frame
  #' #'  Start with predators
  #' Predicted_rsf <- as.data.frame(combo_covs) %>%
  #'   dplyr::select(obs, Area, x, y) %>% 
  #'   mutate(Area = ifelse(Area == 0, "Northeast", "Okanogan")) %>%
  ...
  #' predicted_rsf <- Predicted_rsf_OK_rescale[,c(3:4,7:8)]
